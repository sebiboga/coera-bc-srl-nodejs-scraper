...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-coera-jobs.js`. |
...
| `tests/company.json` | Mock ANAF company data for COERA used in unit tests |
| `tests/validate-coera-jobs.js` | **COERA-specific fast validator (used by CI).** HEAD requests only, hardcoded COERA CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real COERA API, ANAF, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for COERA Careers |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 32519996. Use only when you need to wipe COERA jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape COERA → transform → upsert SOLR → generate docs/jobs.md