...
describe('src/anaf.js', () => {
  ...
  describe('searchCompany', () => {
    it('should return array of companies for valid brand', async () => {
      mockFetch.mockResolvedValue(anafSearchResponse([
        { cui: 32519996, name: 'COERA BC SRL', statusLabel: 'Funcțiune' }
      ]));
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(anaf.searchCompany('COERA')).rejects.toThrow('ANAF search error: 500');
    });
    ...
    it('should encode brand name in URL', async () => {
      ...
      await anaf.searchCompany('COERA SRL');
      expect(capturedUrl).toContain(encodeURIComponent('COERA SRL'));
    });
  });
  ...
  describe('getCompanyFromANAF', () => {
    it('should return company data for valid CIF', async () => {
      ...
      const data = await anaf.getCompanyFromANAF('32519996');
    ...
    it('should throw after exhausting retries', async () => {
      ...
      await expect(anaf.getCompanyFromANAF('32519996')).rejects.toThrow();
  ...
  describe('getCompanyFromANAFWithFallback', () => {
    it('should return fresh data when API works', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('32519996');
    ...
    it('should use cached data when API fails', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('32519996', CACHED_DATA);
    ...
    it('should throw when API fails and no cache available', async () => {
      ...
      await expect(anaf.getCompanyFromANAFWithFallback('32519996')).rejects.toThrow();
    });
  });
});
